...
export async function getReturnTicketRules(): Promise<ReturnTicketRules> {
  ...
  if (!config) {
    return { thirtyDayOffsetDays: FALLBACK_THIRTY_DAY_OFFSET_DAYS, sixtyDayOffsetDays: FALLBACK_SIXTY_DAY_OFFSET_DAYS };
  }
  return { thirtyDayOffsetDays: config.thirtyDayOffsetDays, sixtyDayOffsetDays: config.sixtyDayOffsetDays };
}
