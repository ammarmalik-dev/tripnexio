 ...
 *
 * Exported as a plain object schema (`returnTicketFieldsSchema`) BEFORE the
 * cross-field `.refine()` is applied, so callers that need one field's own
 ...
 * `returnTicketFieldsSchema.shape.travelDate` — a `.refine()`-wrapped
 * ZodEffects has no `.shape`, only the plain object does.
 */
...
export const returnTicketRequestSchema = returnTicketFieldsSchema.refine(
  (values) => new Date(values.returnDate) >= new Date(values.travelDate),
  { message: "Return date must be on or after the travel date", path: ["returnDate"] }
);
