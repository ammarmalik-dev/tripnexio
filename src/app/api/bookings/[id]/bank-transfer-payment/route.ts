...
import { createPendingBankTransferPayment } from "@/lib/payments/create-bank-transfer-payment";
...
/**
 * Step 51 — the Bank Transfer counterpart to POST /api/bookings/[id]/payments
 * (same preconditions: booking PENDING, no other PENDING payment, quotation
 * selected and not expired), used by the Manual Lead flow's "Bank Transfer"
 * option on the Booking detail page.
 */
export async function POST(_request: NextRequest, { params }: RouteParams) {
  ...
  const payment = await createPendingBankTransferPayment({
    booking,
