...
export async function PATCH(request: NextRequest) {
  ...
  const updated = await db.$transaction(async (tx) => {
    const result = await tx.couponConfig.update({ where: { id: COUPON_CONFIG_ID }, data: parsed.data });
    await writeAudit(tx, {
      entityType: "CouponConfig",
      entityId: COUPON_CONFIG_ID,
      action: "UPDATE",
      byUserId: session.id,
      note: `Employee coupon cap updated: ₹${existing.employeeCouponCap} -> ₹${result.employeeCouponCap} (by ${session.name})`,
    });
    return result;
  });
