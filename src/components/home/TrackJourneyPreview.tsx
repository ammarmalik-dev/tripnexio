...
export function TrackJourneyPreview() {
  return (
    <section className="py-8 sm:py-10">
      <Container>
        <MotionReveal>
          <GlassCard tier={2} className="flex flex-col gap-8 p-6 sm:p-10">
            ...
            <div className="relative flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
              <div
                className="absolute left-0 right-0 top-4 hidden h-px bg-hairline sm:block"
                aria-hidden="true"
              />
              {stages.map((stage) => (
                <div
                ...
                >
                  <span
                    className={cn(
                      ...
                      stage.status === "done" && "bg-success/20 text-success",
                      stage.status === "current" &&
                        "bg-[image:var(--gradient-accent)] text-white shadow-[0_8px_20px_-6px_rgb(62_111_219_/_60%)]",
                      stage.status === "upcoming" && "glass-1 text-ink-muted"
                    )}
                  >
                  ...
                  <span
                    className={cn(
                      "text-sm font-medium",
                      stage.status === "upcoming" ? "text-ink-tertiary" : "text-ink-primary"
                    )}
                  >
                    {stage.label}
                  </span>
